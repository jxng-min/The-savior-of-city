...
namespace _State
{
    public class PlayerSkill1State : MonoBehaviour, IPlayerState
    {
        ...
        private void Attack()
        {
            if(m_player_ctrl.m_current_attack_time[0] <= 0f)
            {
                SoundManager.Instance.ButtonSkill1();
                m_player_ctrl.m_animator.SetTrigger("Attack1");
            
                Collider2D[] collider2Ds = Physics2D.OverlapBoxAll(m_player_ctrl.m_attack_pos[0].position, m_player_ctrl.m_attack_box_size[0], 0);
                ...
                m_player_ctrl.m_current_attack_time[0] = m_player_ctrl.m_attack_cool_time[0];
            }
        }
    }
}